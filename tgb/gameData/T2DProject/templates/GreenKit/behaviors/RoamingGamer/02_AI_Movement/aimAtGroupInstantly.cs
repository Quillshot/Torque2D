if (!isObject(aimAtGroupInstantly))
{
   %behavior = new BehaviorTemplate(aimAtGroupInstantly)
   {
      friendlyName = "Aim At Object(s) Instantly";
      behaviorType = "02 - AI Movement";
      description  = "Automatically rotate to face the nearest object in a specified group. (Rotation is instantaneous.)";
   };
...
function aimAtGroupInstantly::onAddToScene(%this, %scenegraph) 
{
   if(%this.debugMode) echo("aimAtGroupInstantly::onAddToScene( ", ((%this.getName() $= "") ? %this : %this.getName()) , " ) @ ", getSimTime() );
...
function aimAtGroupInstantly::aim(%this) 
{
   if(%this.debugMode) echo("aimAtGroupInstantly::aim( ", ((%this.getName() $= "") ? %this : %this.getName()) , " ) @ ", getSimTime() );
   if(!%this.enable) return;
