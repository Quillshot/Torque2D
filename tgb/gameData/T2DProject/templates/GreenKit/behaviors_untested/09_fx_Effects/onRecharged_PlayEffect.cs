if (!isObject(onRechargedPlayEffect)) 
{
   %behavior = new BehaviorTemplate(onRechargedPlayEffect)
   {
...
function onRechargedPlayEffect::onAddToScene(%this, %scenegraph) 
{   
   if(!%this.enable) return;
   if(!%this.debugMode) echo("onRechargedPlayEffect::onAddToScene( ", %this,  " ) @ ", getSimTime() );
}

function onRechargedPlayEffect::onRecharged(%this) 
{
   if(!%this.enable) return;
   if(!%this.debugMode) echo("onRechargedPlayEffect::onRecharged( ", %this,  " ) @ ", getSimTime() );
    
   %this.playEffect();
}

function  onRechargedPlayEffect::playEffect( %this )
{      
   if(!%this.debugMode) echo("onRechargedPlayEffect::playEffect( ", %this,  " ) @ ", getSimTime() );
   %found = false;
