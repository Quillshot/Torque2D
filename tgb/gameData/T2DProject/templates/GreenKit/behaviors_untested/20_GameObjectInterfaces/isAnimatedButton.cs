if (!isObject(isAnimatedButton))
{
   %behavior = new BehaviorTemplate(isAnimatedButton) 
   { 
      friendlyName = "Is Animated Button";
...
function isAnimatedButton::onAddToScene(%this, %scenegraph)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo("isAnimatedButton::onAddToScene()");
   
   %this.owner.setUseMouseEvents(true);
   sceneWindow2D.setUseObjectMouseEvents(1);
}

function isAnimatedButton::onMouseEnter(%this, %modifier, %worldPos)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo("isAnimatedButton::onMouseEnter()");
   %this.owner.setFrame(1);
}

function isAnimatedButton::onMouseLeave(%this, %modifier, %worldPos)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo("isAnimatedButton::onMouseLeave()");
   %this.owner.setFrame(0);
}

function isAnimatedButton::onMouseDown(%this, %modifier, %worldPos)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo("isAnimatedButton::onMouseDown()");
   %this.owner.setFrame(2);
}

function isAnimatedButton::onMouseUp(%this, %modifier, %worldPos)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo("isAnimatedButton::onMouseUp()");
...
function isAnimatedButton::loadNamedLevel(%this)
{
   if(%this.debugMode) echo("isAnimatedButton::loadNamedLevel()");
