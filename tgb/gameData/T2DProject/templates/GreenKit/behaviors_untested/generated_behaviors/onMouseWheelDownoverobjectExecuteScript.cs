...
if (!isObject(onMouseWheelDownoverobjectExecuteScript))
{
   %behavior = new BehaviorTemplate(onMouseWheelDownoverobjectExecuteScript) 
   { 

      friendlyName = "onMouseWheelDown() (over object) -> ExecuteScript";
...
function onMouseWheelDownoverobjectExecuteScript::onAddToScene(%this, %scenegraph)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo(%this.getName() @ "::onAddToScene()");

   //Event Atom Code:onMouseWheelDown() (over object) ->
   %this.owner.setUseMouseEvents(true);
   //sceneWindow2D.setUseObjectMouseEvents(1);

}//onAddToScene

function onMouseWheelDownoverobjectExecuteScript::executeAction(%this)
{
   if(!%this.enable) return;
