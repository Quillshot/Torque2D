...
if (!isObject(onMouseWheelUpanywhereinwindowExecuteScript))
{
   %behavior = new BehaviorTemplate(onMouseWheelUpanywhereinwindowExecuteScript) 
   { 

      friendlyName = "onMouseWheelUp() (anywhere in window) -> ExecuteScript";
...
function onMouseWheelUpanywhereinwindowExecuteScript::onAddToScene(%this, %scenegraph)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo(%this.getName() @ "::onAddToScene()");

   //Event Atom Code:onMouseWheelUp() (anywhere in window) ->
...
function onMouseWheelUpanywhereinwindowExecuteScript::executeAction(%this)
{
   if(!%this.enable) return;
