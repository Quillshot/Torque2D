...
if (!isObject(onMouseMoveanywhereinwindowExecuteScript))
{
   %behavior = new BehaviorTemplate(onMouseMoveanywhereinwindowExecuteScript) 
   { 

      friendlyName = "onMouseMove() (anywhere in window) -> ExecuteScript";
...
function onMouseMoveanywhereinwindowExecuteScript::onAddToScene(%this, %scenegraph)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo(%this.getName() @ "::onAddToScene()");

   //Event Atom Code:onMouseMove() (anywhere in window) ->
...
function onMouseMoveanywhereinwindowExecuteScript::onMouseMove(%this, %modifier, %worldPos)
{
   if(!%this.enable) return;
...
function onMouseMoveanywhereinwindowExecuteScript::executeAction(%this)
{
   if(!%this.enable) return;
