...
if (!isObject(onRightMouseDraggedoverobjectExecuteScript))
{
   %behavior = new BehaviorTemplate(onRightMouseDraggedoverobjectExecuteScript) 
   { 

      friendlyName = "onRightMouseDragged() (over object) -> ExecuteScript";
...
function onRightMouseDraggedoverobjectExecuteScript::onAddToScene(%this, %scenegraph)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo(%this.getName() @ "::onAddToScene()");

   //Event Atom Code:onRightMouseDragged() (over object) ->
   %this.owner.setUseMouseEvents(true);
   //sceneWindow2D.setUseObjectMouseEvents(1);

}//onAddToScene

function onRightMouseDraggedoverobjectExecuteScript::onRightMouseDragged(%this, %modifier, %worldPos)
{
   if(!%this.enable) return;
...
function onRightMouseDraggedoverobjectExecuteScript::executeAction(%this)
{
   if(!%this.enable) return;
