...
if (!isObject(onMouseEnteranywhereinwindowExecuteScript))
{
   %behavior = new BehaviorTemplate(onMouseEnteranywhereinwindowExecuteScript) 
   { 

      friendlyName = "onMouseEnter() (anywhere in window) -> ExecuteScript";
      behaviorType = "Mouse Event -> Scripting";
      description = "When mouse enters window area execute the specified torque script. (Can be any valid Torque Script code.)";
...
function onMouseEnteranywhereinwindowExecuteScript::onAddToScene(%this, %scenegraph)
{
   if(!%this.enable) return;
   if(%this.debugMode) echo(%this.getName() @ "::onAddToScene()");

   //Event Atom Code:onMouseEnter() (anywhere in window) ->
...
function onMouseEnteranywhereinwindowExecuteScript::onMouseEnter(%this, %modifier, %worldPos)
{
   if(!%this.enable) return;
...
function onMouseEnteranywhereinwindowExecuteScript::executeAction(%this)
{
   if(!%this.enable) return;
