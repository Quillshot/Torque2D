...
static void QuakePresent()
{
   if( g.m_inErrorState )
      return;

   if( FAILED( g.m_pD3DX->UpdateFrame( g.m_doFlip ? 0 : D3DX_UPDATE_NOVSYNC ) ) )
      g.m_inErrorState = TRUE;
}

static void QuakeBeginScene()
{
   if( g.m_inErrorState )
      return;
...
{
   SAFE_RELEASE( g.m_fmtvbuf );
   SAFE_RELEASE( g.m_mtvbuf );
   SAFE_RELEASE( g.m_tvbuf );
   SAFE_RELEASE( g.m_vbuf );
   SAFE_RELEASE( g.m_fmtvtgt );
   SAFE_RELEASE( g.m_mtvtgt );
   SAFE_RELEASE( g.m_tvtgt );
   SAFE_RELEASE( g.m_vtgt );
...
{
   // Clear existing state blocks
   for( int i = 0; i < 2; i++ )
      for( int j = 0; j < 10; j++ )
         if( g.m_shaders[i][j] )
         {
            g.m_d3ddev->DeleteStateBlock( g.m_shaders[i][j] );
            g.m_shaders[i][j] = 0;
         }
...
bool reacquireStuff()
{
   // I summon Dave Moore! Dark prince of hacks! Emperor of insufficent dev time!
   //
   // I am really not sure how to tell when it is "ok" to reacquire the resources
   // since DX7 seems to not have any way of letting me know. I'm also not sure
   // what Windows messages I could look for. I could watch login state, however
   // that is WinXP only, and I don't want to cause any kind of compatibility
   // issues this close to TGB launch. This is a really bad solution. -patw
   Sleep( 2500 );

   if( !initVertexBuffers() )
      return false;

   initMaterialsAndCache();
   initStates();
   initStateBlocks();

   return SUCCEEDED( g.m_pD3DX->RestoreSurfaces() );
}
...
{    
    ...
    if(g.m_texturing) 
    {
        if(g.m_mtex)
        {
            ...
            {
                hr = g.m_mtvbuf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_OKTOSWAP, &g.m_verts, 0);
                if( D3DFAILED(hr) )
                {
                  g.m_verts = NULL;
			         return;
            ...
            {
                hr = g.m_mtvbuf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_NOOVERWRITE, &g.m_verts, 0);
                if( D3DFAILED(hr) )
                {
                  g.m_verts = NULL;
			         return;
        ...
        else
        {
            ...
            {
                hr = g.m_tvbuf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_OKTOSWAP, &g.m_verts, 0);
                if( D3DFAILED(hr) )
                {
                  g.m_verts = NULL;
			         return;
            ...
            {
                hr = g.m_tvbuf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_NOOVERWRITE, &g.m_verts, 0);
                if( D3DFAILED(hr) )
                {
                  g.m_verts = NULL;
			         return;
    ...
    else
    {
        ...
        {
            hr = g.m_vbuf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_OKTOSWAP, &g.m_verts, 0);
            if( D3DFAILED(hr) )
            {
               g.m_verts = NULL;
			      return;
        ...
        {
            hr = g.m_vbuf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_NOOVERWRITE, &g.m_verts, 0);
            if( D3DFAILED(hr) )
            {
               g.m_verts = NULL;
			      return;
...
{
   if( g.m_inErrorState || g.m_verts == NULL )
      return;
...
static BOOL fillVB(BOOL process, unsigned min, unsigned vcount)
{
	GLsizei i;
	HRESULT hr;
	...
	if (g.m_usefogary)
	{
		...
		{
			hr = g.m_fmtvbuf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_OKTOSWAP, &g.m_verts, 0);
			if( D3DFAILED(hr) )
			{
			   g.m_verts = NULL;
			   return FALSE;
		...
		{
			hr = g.m_fmtvbuf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_NOOVERWRITE, &g.m_verts, 0);
			if( D3DFAILED(hr) )
			{
			   g.m_verts = NULL;
			   return FALSE;
...
GLAPI void GLAPIENTRY glOrtho (GLdouble left, GLdouble right, GLdouble bottom, GLdouble top, GLdouble zNear, GLdouble zFar)
{
   ...
   // This is in here for the GUI. I assume it corrects the half-pixel offset
   // that D3D has for the fill convention. The problem is that it screws up
   // Torque2D. Thus, a hack must be devised. Since Torque2D always sets zFar
   // to the constant: t2dSceneGraph::maxLayersSupported, and Torque only calls
   // this function from dglMatrix.cc, and all instances use 1.0 as zFar, I will
   // assume that, if zFar is > 1.0, this is a T2D direct call, and so
   // I should forget the fill convention. -pw
   if( !( zFar > 1.f ) )
   {
	   left += 0.5;
	   right += 0.5;
   ...
   // This was changed from -1.0, to -2.0 as per OpenGL docs -pw
	f._33 = (D3DVALUE) (-2.0 / (zFar - zNear));

   // This is wrong, according to the way OpenGL is documented. However if I 
   // "fix" it, then Torque doesn't draw, so I really don't have any idea
   // why this works -pw
...
GLAPI void GLAPIENTRY glDrawArrays (GLenum mode, GLint first, GLsizei count)
{
	...
	if (g.m_lckcount == 0)
	{
		...
		if (g.m_tgtdirty)
			if (textrans)
			{
				if( !fillVB(false,g.m_lckfirst,g.m_lckcount) )
				   return;
			}
			else
			{
				if( !fillVB(true,g.m_lckfirst,g.m_lckcount) )
				   return;
         }
...
GLAPI void GLAPIENTRY glReadPixels (GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type, GLvoid *pixels)
{
   ...
   // [neo, 5/31/2007 - #3175]
   // This should keep at least some people out of danger seeing as none of the other
   // formats/types were implemented.
   if( format != GL_RGB )
   {
      OutputDebugString( "D3D Opengl Wrapper: Unsupported format, this wrapper only support GL_RGB!" );
   ...
   {
      OutputDebugString( "D3D Opengl Wrapper: Unsupported buffer type, this wrapper only support GL_BYTE/GL_UNSIGNED_BYTE!" );
   ...
   LPDIRECTDRAWSURFACE7 lpBufferDDS = g.m_pD3DX->GetPrimary();

	vddsd2.dwSize = sizeof(DDSURFACEDESC2);
	
   lpBufferDDS->GetSurfaceDesc( &vddsd2 );
   ...
   // [neo, 6/7/2007 - #3175]	
   // I don't know what the original intent was here but it fails to do
   // what is needed and is definitely not allowing for x and y.
   ...
   // [neo, 6/7/2007 - #3175]	
   // OpenGL glReadPixels assumes an origin at the bottom left corner of the window
   // The D3D front buffer accesses the entire frame buffer in windowed mode so we
   // need to offset it to within the window bounds. First convert window client 
   // rect to screen space.
   GetClientRect( g.m_hwnd, &src );

   POINT p1 = { src.left, src.top };
   POINT p2 = { src.right, src.bottom };
      
   ClientToScreen( g.m_hwnd, &p1 );
   ClientToScreen( g.m_hwnd, &p2 );

   SetRect( &src, p1.x, p1.y, p2.x, p2.y );

   // Now offset and transform using origing at the bottom left
   src.left  += x;
   src.top    = src.bottom - ( y + height );
   ...
   // Blit the rect we're interested in to temp
   lpDDS->BltFast( 0, 0, g.m_pD3DX->GetPrimary(), &src, DDBLTFAST_WAIT | DDBLTFAST_NOCOLORKEY );	
	
   // Don't need the surface any more
   lpDDS->Release();
   
	// Now copy to the output format
   switch (bpp)
	{
		case 2:
			{
...
GLAPI void* GLAPIENTRY glLockVertexBufferEXT(GLint handle, GLsizei size)
{
   HRESULT hr;
	...
	if (binfo.preserve)
	{
		hr = binfo.buf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY, &binfo.verts, 0);
		if( FAILED(hr) )
		{
		   return NULL;
		}
		...
		{
			hr = binfo.buf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_OKTOSWAP, &binfo.verts, 0);
			if( FAILED(hr) )
			{
			   return NULL;
			}
		...
		{
			hr = binfo.buf->Lock(DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_NOOVERWRITE, &binfo.verts, 0);
			if( FAILED(hr) )
			{
			   return NULL;
			}
...
success:    
    ...
    // Set the initial render states
   initStates();
    
    // Create the state blocks, but first NULL them
    for( int i = 0; i < 2; i++ )
      for( int j = 0; j < 10; j++ )
         g.m_shaders[i][j] = 0;
...
//#define D3DCOLORWRITEENABLE_ALPHA   (1L<<3)
   ...
   g.m_d3ddev->SetRenderState( (D3DRENDERSTATETYPE)168, statevalue );
}
...
GLAPI BOOL GLAPIENTRY wd3dCopyContext(HGLRC src, HGLRC dst, UINT mask)
{
   ...
   HRESULT hr = g.m_dddev->TestCooperativeLevel();
   switch( hr )
   {
      case DDERR_WRONGMODE:   // the surface was created in a different mode, can't restore(but we can destroy/recreate)
      case DD_OK:             // everything is fine
         g.m_inErrorState = false;
         return false;
         break;
      
      default:
         g.m_inErrorState = true;
         return true;        // we are in an error mode still, not safe to destroy/recreate
   }
}
