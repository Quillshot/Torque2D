                                  ...
                                  TextureObject*    to)
{
   ...
   {
	    //handle converting the texture into 16bit
		if(pDL->mForce16Bit) 
		{
			GLint GLformat;
			GLint GLdata_type;
			U32 spanInBytes = (pDL->getWidth( i ) * pDL->getHeight( i )) * (pDL->bytesPerPixel / 4.0);

			//create 16 bit data
			U16 *texture_data = pDL->makeBitmap16Bit(i, pBitmap->getFormat(), &GLformat, &GLdata_type);
...
GBitmap *TextureManager::loadBitmapInstance(const char *textureName, bool recurse /* = true */)
{
   ...
   for (U32 i = 0; i < EXT_ARRAY_SIZE && bmp == NULL; i++)
   {
      ...
      bmp = dynamic_cast <GBitmap *> ((GBitmap*)ResourceManager->loadInstance(fileNameBuffer));
