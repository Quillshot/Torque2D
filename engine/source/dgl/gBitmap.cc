...
U16 *GBitmap::makeBitmap16Bit(S32 iLevel, GBitmap::BitmapFormat alpha_info, GLint *GLformat, GLint *GLdata_type)
{
	U8 *in_source8 = getWritableBits(iLevel);
	U32 width = getWidth(iLevel);
	U32 height = getHeight(iLevel);
	...
	U32 spanInBytes = (width * height) * (bytesPerPixel / 4.0);
