...
bool D3DDevice::smInErrorState = false;
...
bool D3DDevice::setScreenMode( U32 width, U32 height, U32 bpp, bool fullScreen, bool forceIt, bool repaint )
{
   ...
   if ( !newFullScreen && ( newRes.w > winState.desktopClientWidth || newRes.h > winState.desktopClientHeight ) )
   {
      ...
      // Clamp to the desktop client size
      if ( newRes.w > winState.desktopClientWidth )
         newRes.w = winState.desktopClientWidth;
      if ( newRes.h > winState.desktopClientHeight )
         newRes.h = winState.desktopClientHeight;
   }

   //MIN_RESOLUTION defined in platformWin32/platformGL.h
   if ( newRes.w < MIN_RESOLUTION_X || newRes.h < MIN_RESOLUTION_Y )
   {
      Con::warnf( ConsoleLogEntry::General, "D3DDevice::setScreenMode -- can't go smaller than %ix%i!", MIN_RESOLUTION_X, MIN_RESOLUTION_Y );
      return false;
   }
...
#ifdef TORQUE_TOOLS
      winState.appWindow = CreateOpenGLWindow( newRes.w, newRes.h, smIsFullScreen );
#else
      winState.appWindow = CreateOpenGLWindow( newRes.w, newRes.h, smIsFullScreen, false );
#endif
      if ( !winState.appWindow )
      {
   ...
   if ( !newFullScreen )
   {
      ...
      // Clamp the width and height to the desktop client area
      if(adjWidth > winState.desktopClientWidth)
      {
         adjWidth = winState.desktopClientWidth;
         newRes.w = adjWidth;
      }
      if(adjHeight > winState.desktopClientHeight)
      {
         adjHeight = winState.desktopClientHeight;
         newRes.h = adjHeight;
      }
   ...
   // Make sure we feed the correct window size into
   // the Platform layer
   RECT windowRect;
   ...
   Platform::setWindowSize( clientWidth, clientHeight );

   char tempBuf[32];
   dSprintf( tempBuf, sizeof(char) * 32, "%d %d %d", smCurrentRes.w, smCurrentRes.h, smCurrentRes.bpp );
   if( fullScreen )
...
void D3DDevice::swapBuffers()
{
   if( smInErrorState )
   {
      ...
      if( !smInErrorState )
      {
         if( Con::isFunction( "detachMenuBars" )  )
            Con::executef(1, "detachMenuBars" );
         Video::reactivate( true );
         if( Con::isFunction( "attachMenuBars" ) )
            Con::executef(1, "attachMenuBars" );
      }
   }
   else
...
DisplayDevice* D3DDevice::create()
{
   ...
   HWND testWindow = CreateWindow(
      ...
      dT(""),
      WS_POPUP,
      0, 0, MIN_RESOLUTION_X, MIN_RESOLUTION_Y,//MIN_RESOLUTION defined in platformWin32/platformGL.h
      NULL,
      NULL,
