...
t2dTextObject::t2dTextObject() : mConsoleBuffer( NULL ),
                                 ...
                                 mEditing( false ),
								 mUseBilinearFilter(false),
								 mSnapToIntegerPos(false),
								 mNoUnicode(false)
{
...
void t2dTextObject::initPersistFields()
{
   ...
   addProtectedField( "bilinearFilter", TypeBool, Offset( mUseBilinearFilter, t2dTextObject), setBilinearFilter, defaultProtectedGetFn, "Whether or not the font uses bilinear filtering" );
   addProtectedField( "snapToInteger", TypeBool, Offset( mSnapToIntegerPos, t2dTextObject), setSnapToIntegerPos, defaultProtectedGetFn, "Whether or not the font destination is clamped to start on an integer value" );
   addProtectedField( "noUnicode", TypeBool, Offset( mNoUnicode, t2dTextObject), setNoUnicode, defaultProtectedGetFn, "Whether or not the text doesn't need to support Unicode conversions (ie. know that it is just numbers, standard english, etc.)" );
}
...
void t2dTextObject::copyTo(SimObject* object)
{
   ...
   textObject->mUseBilinearFilter = mUseBilinearFilter;
   textObject->mSnapToIntegerPos = mSnapToIntegerPos;
...
void t2dTextObject::renderText( GFont* font )
{
   ...
   for( S32 i = 0; i < mText.length(); i++ )
   {
      ...
      // Texture coords.
      const F32 texLeft = ( F32( ci.xOffset ) + ( ci.width * texOffset.mX ) ) / F32( lastTexture->texWidth );
      const F32 texRight = F32( ci.xOffset + ( ci.width * texScale.mX ) ) / F32( lastTexture->texWidth );
      const F32 texTop = F32( ci.yOffset ) / F32( lastTexture->texHeight );
      const F32 texBottom = F32( ci.yOffset + ( ci.height * texScale.mY ) ) / F32( lastTexture->texHeight );
...
//Luma: returns the actual size that the text takes up
