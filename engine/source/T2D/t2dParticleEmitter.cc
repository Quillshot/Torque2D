...
void t2dParticleEmitter::integrateObject( const F32 sceneTime, const F32 elapsedTime, CDebugStats* pDebugStats )
{
    ...
    if ( !mPauseEmitter )
    {
        ...
        else
        {
            ...
            const F32 effectEmission = pParentEffectObject->mQuantity.t2dGraphField_Base.getGraphValue( effectAge ) * Con::getFloatVariable("$pref::T2D::particleEngineQuantityScale", 1.0f);
...
void t2dParticleEmitter::renderObject( const RectF& viewPort, const RectF& viewIntersection )
{
    ...
    if ( mAttachPositionToEmitter )
    {
        ...
        const t2dVector effectPos = pParentEffectObject->getRenderPosition();
