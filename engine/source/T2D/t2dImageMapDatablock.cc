												...
												mForce16bit(0)
{
    ...
    mPreload = Con::getBoolVariable( "$pref::T2D::imageMapPreloadDefault", true );
...
void t2dImageMapDatablock::unlockImageMap( void )
{
    ...
    if ( mAllowUnload && mLockReference == 0 )
    {
...
void t2dImageMapDatablock::destroyResources()
{
    // Unload Textures.
    unloadTextures();
...
bool t2dImageMapDatablock::loadTextures( bool resurrection )
{
    ...
    // Fetch Image-Map Dump Flag.
	 const bool imageMapDump = (resurrection || mImageMapDumpComplete) ? false : Con::getBoolVariable( "$pref::T2D::imageMapDumpTextures", false );
    ...
    for ( U32 n = 0; n < mVecTexturePage.size(); n++ )
    {
        ...
        if ( imageMapDump )
        {
            ...
            dSprintf( buffer, 256, "imageMapDump/%d_Page_%d.png", getId(), n );
      ...
      dSprintf( nameBuffer, 256, "%d_%d", getId(), n );
...
// Unload Textures.
// --------------------------------------------------------------------------------------
void t2dImageMapDatablock::unloadTextures()
{
    // Link Mode?
    if ( mImageMode != T2D_IMAGEMODE_LINK )
    {
...
void t2dImageMapDatablock::fitFrames( typeFrameIn& vecFrameIn, typeFrameOut& vecFrameOut, cTexturePage& texturePage, U32& totalWastedSpace )
{
    ...
    // Reset Best Fit.
    U32 bestWidth = mMaximumCurrentTextureSize;
    U32 bestHeight = mMaximumCurrentTextureSize;
...
void t2dImageMapDatablock::calculatePackFrameWaste( const typeFrameIn& vecFrameIn, const U32 areaWidth, const U32 areaHeight, U32& wastedSpace, U32& fitOnPage )
{
    ...
    F32 texelWidth = 1.0f / areaWidth;
    F32 texelHeight = 1.0f / areaHeight;
...
void t2dImageMapDatablock::setTextureClamp( const bool s, const bool t )
{
    ...
    for ( S32 n = 0; n < mVecTexturePage.size(); n++ )
    {
        ...
        // Set Clamp Mode.
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, s ? (dglDoesSupportEdgeClamp() ? GL_CLAMP_TO_EDGE_EXT : GL_CLAMP) : GL_REPEAT );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, t ? (dglDoesSupportEdgeClamp() ? GL_CLAMP_TO_EDGE_EXT : GL_CLAMP) : GL_REPEAT );
    }
}
...
//-----------------------------------------------------------------------------
// Get Force16Bit Description.
//-----------------------------------------------------------------------------
ConsoleMethod(t2dImageMapDatablock, getForce16Bit, bool, 2, 2, "() Get Force 16 bit flag.\n"
			  "@return Returns the flag as a boolean")
{
    // Get Filter Mode Description.
