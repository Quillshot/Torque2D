...
void t2dAnimationDatablock::initPersistFields()
{
	...
	addField("animationPingPong",	TypeBool,   Offset(mPingPong,				t2dAnimationDatablock));
	addField("animationReverse",	TypeBool,   Offset(mReverse,				t2dAnimationDatablock));
}
...
bool t2dAnimationController::playAnimation( const char* animationName, const bool autoRestore, S32 startFrame, const bool mergeTime )
{
	...
	//set playing in reverse or not
	mCurrentReverse = mConfigDataBlock->mReverse;
	...
	if(mConfigDataBlock->mReverse) 
	{
		//reverse inverts timing on animation
		mCurrentTime = mTotalIntegrationTime - mCurrentTime;
	}
...
bool t2dAnimationController::updateAnimation( const F32 elapsedTime )
{
    ...
    if ( !mConfigDataBlock->mAnimationCycle )
    {
		if((!mCurrentReverse && mGreaterThanOrEqual(mCurrentTime, mTotalIntegrationTime)) ||
			(mCurrentReverse && mLessThanOrEqual(mCurrentTime, 0)))
		{
			//ping pong animations only end when they have "ponged"
			if(!mConfigDataBlock->mPingPong || (mCurrentReverse != mConfigDataBlock->mReverse))
			{
	...
	{
		//mod doesn't play nice with negatives, so if looping and negative, we need to jump back to far end of animation
		if(mCurrentTime < 0.0f)
		{
			mCurrentTime += mTotalIntegrationTime;
		}
	...
	if(mConfigDataBlock->mPingPong)
	{
		if(mGreaterThanOrEqual(mCurrentTime, mTotalIntegrationTime))
		{
			mCurrentFrameIndex = mMaxFrameIndex;
		}
		else if(mLessThanOrEqual(mCurrentTime, 0.0f))
		{
			mCurrentFrameIndex = 0;
		}
		else
	...
	if(mConfigDataBlock->mPingPong)
	{
		...
		else if(mCurrentReverse && mLessThanOrEqual(mCurrentTime, 0.0f))
		{
			mCurrentReverse = false;
			mCurrentTime = 0.0f;
		}
...
{
    // Rest Time.
	mCurrentTime = (mConfigDataBlock->mReverse ? mTotalIntegrationTime : 0.0f);
}
