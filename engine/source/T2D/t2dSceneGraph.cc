...
static const F32 scmContainerBinSize    = 20.0f;
static const U32 scmContainerBinCount   = 256;
...
//-----------------------------------------------------------------------------
// Resize Container Bins.
//-----------------------------------------------------------------------------
ConsoleMethod(t2dSceneGraph, resizeContainerBins, void, 4, 4, "(containerBinSize, containerBinCount) Resize Container Bins.\n"
			  "@param containerBinSize The desired size of the scene graph's bins.\n"
			  "@param containerBinCount The number of bins to create\n"
			  "@return No return value.")
{
    // Initialise.
    object->resizeContainerBins( dAtof(argv[2]), dAtoi(argv[3]) );
}
// Resize Container Bins.
void t2dSceneGraph::resizeContainerBins( const F32 containerBinSize, const U32 containerBinCount )
{
    // Are there any objects in the scene.
    if ( getSceneObjectCount() > 0 )
    {
        // Yes, so temporarily remove scene objects from container system...
        ...
        // Add scene objects back into the new container system...
    ...
    {
        // No, so just initialise Container System
        mSceneContainer.initialise( this, containerBinSize, containerBinCount );
    }
}
...
void t2dSceneGraph::detachAllSceneWindows( void )
{
    ...
    {
       t2dSceneWindow* window = dynamic_cast<t2dSceneWindow*>(mAttachedSceneWindows[mAttachedSceneWindows.size()-1]);

       if (window)
         window->resetSceneGraph();
       else
         mAttachedSceneWindows.removeObject( mAttachedSceneWindows[mAttachedSceneWindows.size()-1] );
    }
}
...
void t2dSceneGraph::processTick( void )
{
       ...
       if ( !getIsEditorScene() )
       {
        ...
        while ( pSceneObject2D != getProcessHead() )
        {
           ...
           if ( !pSceneObject2D->getInitialUpdate() ||
            ...
            {
                // Post-Update.
                pSceneObject2D->integrateObject( mSceneTime, ITickable::smTickSec, pDebugStats );
            }   
    ...
    if ( pDebugStats->objectPotentialCollisions != 0 )
        pDebugStats->objectHitPercCollisions = mDebugStats.objectActualCollisions * ( 100.0f / pDebugStats->objectPotentialCollisions  );
    else
        pDebugStats->objectHitPercCollisions = 100.0f;

    // Relocations...
    if (pDebugStats->objectBinRelocations > mMaxObjectBinRelocations )
        mMaxObjectBinRelocations = pDebugStats->objectBinRelocations;

    // Collisions.
    if ( pDebugStats->objectBinCollisions > mMaxObjectBinCollisions )
        mMaxObjectBinCollisions = pDebugStats->objectBinCollisions;
...
void t2dSceneGraph::renderView( const RectF viewWindow, const U32 layerMask, const U32 groupMask, CDebugStats* pDebugStats )
{
    ...
    if ( mSceneContainer.findObjects( viewWindow, layerMask, groupMask, false, false, findLayeredObjectsCallback, &mLayeredRenderList, NULL ) > 0 )
    {
        ...
        PROFILE_END();   // T2D_t2dSceneGraph_renderObjects
                ...
                // Draw batched quads here.
                t2dQuadBatch::renderBatches();
            }
        }
