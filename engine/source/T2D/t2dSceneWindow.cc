...
//-----------------------------------------------------------------------------
void t2dSceneWindow::sendObjectMouseEvent( const char* name, const GuiEvent& event )
{
...
void t2dSceneWindow::onMouseDown( const GuiEvent& event )
{
    ...
    dispatchMouseEvent("onMouseDown", event);
}
...
// T2D Debug Profiling.
    ...
    if ( pT2DSceneGraph->getDebugMask() & t2dSceneGraph::T2D_SCENEGRAPH_DEBUG_STATISTICS )
    {
        ...
        // Get Debug Stats.
        const CDebugStats& lastDebugStats = pT2DSceneGraph->getLastDebugStats();
        ...
        // Scene Timing.
        dSprintf( mDebugText, sizeof( mDebugText ), "SceneTime: %7.1f / ActualFPS: %7.1f", pT2DSceneGraph->getSceneTime(), Con::getFloatVariable("fps::real",0.0f) );
        ...
        // Container System.
        dSprintf( mDebugText, sizeof( mDebugText ), "BinReloc : %7.1d / MaxReloc : %7.1d / BinCollis: %7.1d  / MaxCollis: %7.1d / BinSearch: %7.1d", lastDebugStats.objectBinRelocations, pT2DSceneGraph->getMaxBinRelocations(), lastDebugStats.objectBinCollisions, pT2DSceneGraph->getMaxBinCollisions(), lastDebugStats.objectBinSearches );
        dglDrawText( mpDebugFont, bannerOffset + Point2I(0,mpDebugFont->getHeight()*6), mDebugText, NULL );

        // Collisions.
        dSprintf( mDebugText, sizeof( mDebugText ), "PotCol   : %7.1d / ActCol   : %7.1d / ColHit   : %7.1f%% / Contacts : %7.1d /", lastDebugStats.objectPotentialCollisions, lastDebugStats.objectActualCollisions, lastDebugStats.objectHitPercCollisions, lastDebugStats.objectContactPairs );
        dglDrawText( mpDebugFont, bannerOffset + Point2I(0,mpDebugFont->getHeight()*7), mDebugText, NULL );

        // Rendering.
        dSprintf( mDebugText, sizeof( mDebugText ), "PotRender: %7.1d / ActRender: %7.1d / RenderHit: %7.1f%% / SortedObj: %7.1d /", lastDebugStats.objectsPotentialRender, lastDebugStats.objectsActualRendered, lastDebugStats.objectsHitPercRendered, lastDebugStats.objectsLayerSorted );
        dglDrawText( mpDebugFont, bannerOffset + Point2I(0,mpDebugFont->getHeight()*8), mDebugText, NULL );

        // Particles.
        dSprintf( mDebugText, sizeof( mDebugText ), "ParFree  : %7.1d / ParUsed  : %7.1d /", lastDebugStats.objectParticlesAvailable, lastDebugStats.objectParticlesActive );
