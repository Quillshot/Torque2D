...
bool t2dPhysics::calculateCollision( F32 elapsedTime, cCollisionStatus* pCollisionStatus )
{
    ...
    switch( srcPhysics.getCollisionDetectionMode() )
    {
        ...
        case T2D_DETECTION_CUSTOM:
        {
			...
			AssertWarn(pCollisionStatus->mSrcObject, "t2dPhysics::calculateCollision() - Not implemented, no source object for 'T2D_Detection_Custom'!");
			
			if(!pCollisionStatus->mSrcObject)
				return false;
...
//-----------------------------------------------------------------------------
// Get Supporting Corner Normal.
//-----------------------------------------------------------------------------
void t2dPhysics::getSupportingCornerNormal( const t2dVector* pVertices, const U32 numVertices, const U32 index, const t2dVector objectPosition, t2dVector& collisionNormal )
{
    // Sanity!
    AssertFatal( index >= 0 && index < numVertices, "t2dPhysics::getSupportingCornerNormal() - Invalid Vertex Index!" );

    // Fetch Indexed Vertex.
    const t2dVector vIndex = pVertices[index];

	// Calculate Vertex/Object Direction.
	const t2dVector vertexObjDirection = objectPosition - vIndex;

    // Is this a line?
    if ( numVertices == 2 )
    {
		// Yes, so fetch edge normal.
		const t2dVector edgeNormal = (pVertices[1] - pVertices[0]).getPerp();

		// Normal Edge Region?
		if ( vertexObjDirection.dot( edgeNormal ) < 0 )
		{
			collisionNormal = edgeNormal;
		}
		// No, so opposite Edge Region.
		else
		{
			collisionNormal = -edgeNormal;
		}
    ...
    // Calculate Backwards Edge (directed towards vertex)
    if ( index == 0 )
    {
        vBackEdge = vIndex - pVertices[numVertices-1];
    }
    else
    {
        vBackEdge = vIndex - pVertices[index-1];
    }

    // Calculate Forwards Edge (directed towards vertex)
	...
	// Calculate Edge/Vertex Voronoi planes...
	const t2dVector backEdgeVeroniPlane = -vBackEdge.getPerp();
	const t2dVector forwardEdgeVeroniPlane = vForwardEdge.getPerp();

	// Calculate Region Occupancy.
	const bool backVeronoiRegionOccupancy = vertexObjDirection.dot( backEdgeVeroniPlane ) > 0;
	const bool forwardVeronoiRegionOccupancy = vertexObjDirection.dot( forwardEdgeVeroniPlane ) > 0;

	// back-Edge Voronoi region?
	if ( backVeronoiRegionOccupancy )
	{
		// Yes, so are we in the forward-edge Veronoi region as well?
		if ( forwardVeronoiRegionOccupancy )
		{
			// Yes, so we're in the vertex Voronoi region.
			collisionNormal = (vBackEdge + vForwardEdge).getPerp();
		}
		else
		{
			// No, so we're in the back-edge Voronoi region.
			collisionNormal = backEdgeVeroniPlane;
		}
	}
	else
	{
		// No, so we're in the forward-edge Voronoi region.
		collisionNormal = forwardEdgeVeroniPlane;
	}
}
...
bool t2dPhysics::sweptCircleEdgeVertexIntersect( const t2dVector& spaceXForm, const t2dVector* pVertices, U32 numVertices, const t2dVector& begin, const t2dVector& delta, F32 A, F32 B, F32 C, cCollisionStatus* pCollisionStatus )
{
    ...
    for ( const t2dVector *pv1 = pVertices, *pv2 = pVertices + numVertices - 1; vertex < numVertices; pv2 = pv1, ++pv1, ++vertex )
    {
        ...
        if ( findLowestRootInInterval(a1, b1, c1, upperTime, time) )
        {
			...
			// Calculate Circle Position at Collision Time.
			const t2dVector circlePosAtCollision = begin + (delta * time);

            // Get Supporting Corner Collision Normal.
			getSupportingCornerNormal( pVertices, numVertices, vertex, circlePosAtCollision, colNormal );
...
void t2dPhysics::moveToCollisionPoint( cCollisionStatus* pCollisionStatus )
{
    ...
    if ( !pCollisionStatus->mOverlapped )
    {
        ...
        backOff.normalise(T2D_CONST_COLLISION_MARGIN * 2.0f);
        spatialStep -= backOff;
...
void t2dPhysics::resolveOverlapPair( cCollisionStatus* pCollisionStatus, const U32 contactIndex )
{
    ...
    overlapExtend.normalise(T2D_CONST_COLLISION_MARGIN * 10.0f);
...
bool t2dPhysics::castLine( const t2dVector& startPos, const t2dVector& endPos, t2dVector& collisionNormal, F32& collisionTime )
{
    ...
    switch( getCollisionDetectionMode() )
    {
		...
		case T2D_DETECTION_CUSTOM:
        {
			AssertWarn(true, "t2dPhysics::calculateCollision() - Custom detection not implemented!");
...
void t2dPhysics::processPositionTarget( void )
{
    ...
    if ( getPositionTargetAutoStop() ) setGrossLinearVelocity( t2dVector::getZero() );
    // Snap Position (if selected).
    if ( getPositionTargetSnap() ) setPosition( getPositionTarget() );
